...
In this repository, each folder contains the results of the composite analysis (subfolder: composite_thermals.zip), and the Python codes used (subfolder: scripts), all run using the scripts in subfolder: jobs. 
...
The results of the tracking algorithm for each event (subfolder: thermals_all) and the interpolated WRF grid are not available in the repository due to storage restrictions but can be shared upon request.
