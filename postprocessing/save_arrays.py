...
"""
# Save arrays of a certain variable, 80 km from AWS, during certain dates, and coarsen 200 m data to 1 km (keeping ONE dimension - vertical dimension defined by "how")
#
# Inputs:
...
#
# Outputs:
# {case}_d03_{var}{how}_80kmAWS_originalgrid.npy
# {case}_d02_{var}{how}_80kmAWS_coarsen.npy
# {case}_d03_{var}{how}_80kmAWS_coarsen.npy
"""
...
def save_arrays(case, var, hows = ["All"]):
    ...
    for how in hows:
        ...
        nans_d03 = np.isnan(arr_d03)
        arr_d03 =  arr_d03[~nans_d03]
        np.save(f'{path_arrays}/{case}_d03_{var}{how}_80kmAWS_originalgrid.npy', arr_d03)
        ...
        arr_d02         = aux_d02.to_numpy().flatten()
        arr_d03_coarsen = aux_d03_coarsen.to_numpy().flatten()
        
        nans_d02 = np.isnan(arr_d02)
        nans_d03 = np.isnan(arr_d03_coarsen)
        nans_both = (nans_d02 | nans_d03)
        
        arr_d02         =  arr_d02[~nans_both]
        arr_d03_coarsen =  arr_d03_coarsen[~nans_both]
    
        np.save(f'{path_arrays}/{case}_d02_{var}{how}_80kmAWS_coarsen.npy', arr_d02)
        np.save(f'{path_arrays}/{case}_d03_{var}{how}_80kmAWS_coarsen.npy', arr_d03_coarsen)
...
# Save max gust
save_arrays(case, "WSPD10MAX", how = "All")

# Reflectivity
save_arrays(case, "REFD_MAX", hows = ["All"])

# Surface rain
save_arrays(case, "RAINNC", hows = ["All"])
